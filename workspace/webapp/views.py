...
def update_players_score(weekgiven):
    matches = Matches.objects.get( week=weekgiven , status = 'true' , updated = 'false' )
    for match in matches:

        results = Results.objects.get(match_id = match.id)
        for result in results:
            points =  (result.yc * -2) + (result.rc * -5) + (result.assist * 3) + (result.appearance) + (result.goals * 5) + (result.cleanSheet) +  (result.own_goal * -2)
